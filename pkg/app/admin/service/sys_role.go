...
// GetList 获取SysRole list
func (e *SysRole) GetList(list *[]models.SysRole) error {
	var err error
	var data models.SysRole
	err = e.Orm.Model(&data).Preload("SysMenu").Find(list).Error
	if err != nil {
...
func (e *SysRole) Get(d *dto.SysRoleGetReq, model *models.SysRole) error {
	...
	model.MenuIds, err = e.GetRoleMenuId(model.Id)
	if err != nil {
...
func (e *SysRole) Insert(c *dto.SysRoleInsertReq, cb *casbin.SyncedEnforcer) error {
	...
	err = e.Orm.Preload("SysApi").Where("id in ?", c.MenuIds).Find(&dataMenu).Error
	if err != nil {
		e.Log.Errorf("db error:%s", err)
		return err
	}
...
func (e *SysRole) Update(c *dto.SysRoleUpdateReq, cb *casbin.SyncedEnforcer) error {
	...
	tx.Preload("SysApi").Where("id in ?", c.MenuIds).Find(&mlist)
...
func (e *SysRole) UpdateDataScope(c *dto.RoleDataScopeReq) *SysRole {
	...
	var model = models.SysRole{}
	tx.Preload("SysDept").First(&model, c.Id)
