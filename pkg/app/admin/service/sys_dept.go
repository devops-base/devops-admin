...
// GetList 获取部门列表
func (e *SysDept) GetList(list *[]models.SysDept) error {
	var err error
...
func (e *SysDept) Insert(c *dto.SysDeptInsertReq) error {
	...
	deptPath := pkg.IntToString(data.Sort) + "/"
	if data.ParentId != 0 {
...
func (e *SysDept) Update(c *dto.SysDeptUpdateReq) error {
	...
	deptPath := pkg.IntToString(model.Id) + "/"
	if model.ParentId != 0 {
		var deptP models.SysDept
...
func (e *SysDept) SetDeptPage(c *dto.SysDeptGetPageReq, deps *[]models.SysDept) *SysDept {
	var list = make([]models.SysDept, 0)
	err := e.getList(c, &list)
	if err != nil {
		_ = e.AddError(err)
		return e
