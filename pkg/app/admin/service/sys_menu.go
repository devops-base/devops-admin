...
func (e *SysMenu) initPaths(tx *gorm.DB, menu *models.SysMenu) error {
	...
	if menu.ParentId != 0 {
		...
		menu.Paths = parentMenu.Paths + "/" + pkg.IntToString(menu.Id)
	} else {
		menu.Paths = "/0/" + pkg.IntToString(menu.Id)
	}
...
func (e *SysMenu) Remove(d *dto.SysMenuDeleteReq) *SysMenu {
	...
	// 检查删除菜单是否包含子菜单.
	state, err := e.checkMenuParent(d.Id)
	if state && err != nil {
		e.Log.Errorf("checkMenuParent error: %v", err)
		_ = e.AddError(err)
		return e
	}
...
func (e *SysMenu) checkMenuParent(id int) (bool, error) {
	// 判断是否存在子菜单,如果存在子菜单,不能删除父菜单.
	var data models.SysMenu
	var count int64
	err := e.Orm.Model(&data).Where("parent_id = ?", id).Count(&count).Error
	if err != nil {
		e.Log.Errorf("check delete sysMenus error %s", err)
		return true, err
	}
	// 如果条数大于0,说明当前菜单还存在子菜单,不能删除..
	if count > 0 {
		err = errors.New("当前菜单存在子菜单,请先删除子菜单在删除父菜单!!")
		//_ = e.AddError(err)
		return true, err
	}
	return false, nil
